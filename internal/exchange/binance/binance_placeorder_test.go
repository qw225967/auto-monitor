...
// TestBinance_PlaceOrder 测试合约下单功能
func TestBinance_PlaceOrder(t *testing.T) {
	// 使用测试辅助函数设置代理
	...
	bn := NewBinance(cfg.Binance.APIKey, cfg.Binance.SecretKey)
	...
	t.Logf("⚠️  Check Binance Futures account for order ID: %s", order.OrderID)
}

// TestBinance_PlaceSpotOrder 测试现货下单功能
func TestBinance_PlaceSpotOrder(t *testing.T) {
	// 使用测试辅助函数设置代理
	defer test.SetupProxyForTest("http://127.0.0.1:7897")()
	...
	t.Logf("✅ Order ID: %s, Status: %s", order.OrderID, order.Status)
	t.Logf("⚠️  Check Binance Spot account for order ID: %s", order.OrderID)
}

// TestBinance_PlaceSpotOrder_Limit 测试现货限价单
func TestBinance_PlaceSpotOrder_Limit(t *testing.T) {
	// 使用测试辅助函数设置代理
	defer test.SetupProxyForTest("http://127.0.0.1:7897")()
